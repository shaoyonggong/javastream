 ...
 */
@Slf4j
public class BeanHandleUtils {


  /**
   * 将指定的List<D>转换为List<T>
   */
   ...
   * 将指定的List<D>转换为List<T>
   */
  public static <T> List<T> batchTransform(final Class<T> clazz, List<? extends Object> srcList,Consumer<T> consumer) {
    if (CollectionUtils.isEmpty(srcList)) {
      return Collections.emptyList();
    }
    ...
    List<T> result = new ArrayList<>(srcList.size());
    for (Object srcObject : srcList) {
      result.add(transfrom(clazz, srcObject,consumer));
    }
    return result;
  }

  /**
   * 封装{@link BeanUtils#copyProperties(Object, Object, String...)}
   * ，惯用与直接将转换结果返回
   *
   * <pre>
  ...
  public static <T> T transfrom(Class<T> clazz, Object src, Consumer<T> consumer) {
    T t = transfrom(clazz, src);
    consumer.accept(t);
    return t;
  }
